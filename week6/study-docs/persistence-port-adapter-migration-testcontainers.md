...
> 작성일: 2026-09-21
> 상태: Ready — 설계 학습자료이며 PostgreSQL 구현·실행 근거가 아님
> 관련 계획: [Week 6 학습 계획](../weekly-plan.md)
> 핵심 질문: Application의 저장 계약을 유지하면서 In-memory 구현을 실제 PostgreSQL 구현으로 어떻게 교체하고 검증할 것인가?

## 현재 Helpdesk Source Baseline

2026-09-21 읽기 전용 Source Audit에서 확인한 현재 구조다.
...
현재 `InMemoryTicketRepository`는 `HashMap`과 `nextId`로 이 계약을 구현한다. Application Service는 구체적인 `HashMap`을 알지 않고 `TicketRepository`에만 의존한다.

현재 Source에는 다음 항목이 없다.

- Spring JDBC 또는 JPA 의존성
- PostgreSQL Driver
- Schema Migration 도구와 SQL File
- PostgreSQL Repository Adapter
- Testcontainers Database Integration Test

따라서 기존 Java Test 통과는 In-memory Application 회귀 근거일 뿐 실제 PostgreSQL Adapter 근거가 아니다.

## Port와 Adapter

### Port

Port는 Application이 외부 저장소에 요구하는 동작을 표현한 계약이다. 현재 `TicketRepository` Interface가 이 역할을 한다.
...
현재 `InMemoryTicketRepository`에는 `@Repository`가 붙어 있다. PostgreSQL 구현에도 바로 `@Repository`를 붙이면 `TicketRepository` Type의 Bean이 두 개가 될 수 있다.
...
이번 Lab에서는 “실제 Runtime은 PostgreSQL, 독립적인 단위 Test는 필요한 경우 In-memory 또는 Test Double”이라는 목적이 드러나는 명시적 조립을 우선 검토한다. 정확한 Annotation과 Profile은 구현 직전 Test 구조까지 보고 결정한다.
...
| 현재 작은 Scope | 생성·단건 조회를 직접 추적하기 쉬움 | 관계가 없으면 ORM 핵심 문제를 관찰하기 어려움 |

현재 Helpdesk에는 Ticket 생성과 단건 조회만 있고 ORM 관계나 N+1을 재현할 Domain 관계가 없다. 반면 Week 3에서 학습한 SQL·Constraint·Transaction과 Row Mapping을 실제 Code에서 확인해야 한다.

따라서 현재 우선 후보는 Spring JDBC다. 이것은 아직 구현 완료 결정이 아니다. 다음 질문에 답한 뒤 최종 선택한다.

1. 직접 작성한 SQL과 Parameter Binding을 관찰하는 것이 이번 학습 목표인가?
2. 지금 JPA 관계 Mapping으로 검증할 실제 Domain 관계가 있는가?
...
현재 `new Ticket(title)`은 항상 `status = OPEN`으로 시작한다.
...
현재 `Ticket` Domain 객체에는 ID가 없고 `save`가 `long` ID를 반환한다. `findById` 뒤 Application Service는 조회에 사용한 ID와 반환된 `Ticket`을 조합해 `TicketResult`를 만든다.

이 계약을 이번 Scope에서 그대로 유지할지, ID를 Domain Identity로 포함할지는 별도 설계 문제다. PostgreSQL을 붙인다는 이유만으로 공개 Port를 먼저 크게 바꾸지 않는다. 최소 Adapter로 현 계약이 가능한지 먼저 검증한다.
...
Migration은 Application Entity에서 Runtime마다 자동으로 DDL을 추측해 만드는 것과 목적이 다르다. 이번 Lab에서는 빈 Database에서 같은 Schema를 재현하는 SQL 근거를 남긴다.
...
현재 Ticket 생성은 단일 `INSERT`이므로 “INSERT가 실패했다”만으로 부분 반영 Rollback을 충분히 보여 주기 어렵다. 의미 있는 Rollback Test에는 하나의 Use Case 안에서 둘 이상의 변경이 있거나, 성공한 변경 뒤 실패하도록 통제된 경계가 필요하다.
...
아직 실행하지 않은 설계 후보이며, 실행 뒤에만 완료로 바꾼다.
...
```text
1. 현재 Port와 Domain 복원 문제 확인
        ↓
2. Spring JDBC / JPA 선택 Gate 통과
        ↓
...
Driver, Flyway와 Testcontainers의 정확한 Artifact 조합은 현재 Spring Boot Version의 공식 문서를 구현 직전에 확인한다. 기억에 의존해 Version을 직접 고정하지 않는다.

## 구현 전에 답할 설계 질문

1. 현재 `TicketRepository`는 Port인가, PostgreSQL 전용 Interface인가?
2. `TicketApplicationService`가 `JdbcTemplate`을 직접 가져서는 안 되는 이유는 무엇인가?
3. 두 Repository 구현이 모두 Bean이면 어떤 문제가 생기는가?
4. 현재 Domain에서 Spring JDBC가 JPA보다 작은 학습 근거를 만드는 이유는 무엇인가?
...
| Row를 읽어 생성자에 넣으면 Domain 복원이 끝난다. | 현재 생성자는 Status를 `OPEN`으로 초기화하므로 복원 경계가 필요하다. |
...
- 이 문서는 설계 학습자료이며 Adapter 구현 근거가 아니다.
- 의존성을 추가한 사실만으로 PostgreSQL 연결을 완료했다고 하지 않는다.
- Migration File 작성만으로 빈 Database 적용을 확인했다고 하지 않는다.
- Testcontainers를 설정한 사실만으로 Integration Test가 통과했다고 하지 않는다.
- In-memory Test 수를 PostgreSQL Test 수로 보고하지 않는다.
- 실제 Test 결과에는 Test 수·실패·오류·건너뜀과 사용한 Database 종류를 함께 기록한다.
